...
def createBuild(sign) {
    ...
    if (sign) {
        dir("artifact-deployer") {
            ...
            sh(script: """#!/bin/bash
                ./sign-zextras-zip ../pkg/com_zextras_zapp_contacts.zip
            """)
        }
    }
}
...
pipeline {
	...
	environment {
		...
		TRANSLATIONS_REPOSITORY_NAME = "zextras/com_zextras_zapp_contacts"
	}
	stages {
		...
		stage("Release automation") {
            ...
            steps {
                ...
                createBuild(true)
                archiveArtifacts(
                    artifacts: "pkg/com_zextras_zapp_contacts.zip",
                    fingerprint: true
                )
                stash(
                    includes: "pkg/com_zextras_zapp_contacts.zip",
                    name: 'zimlet_package'
                )
                ...
                script {
                    ...
                    doc.upload(
                        file: "docs/website/build/com_zextras_zapp_contacts/**",
                        destination: "iris/zapp-contacts/$BRANCH_NAME"
                    )
                }
			...
			parallel {
				stage("Ubuntu") {
					...
					steps {
						...
						sh "docker exec -t ${env.CONTAINER1_ID} bash -c \"cd /u; ./build-pkgs.sh contacts\""
				...
				stage("CentOS") {
					...
					steps {
						...
						sh "docker exec -t ${env.CONTAINER2_ID} bash -c \"cd /r; ./build-pkgs.sh contacts\""
